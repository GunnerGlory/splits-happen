...
def splits_happen(line):
    ...
    if len(line) > 2:
        if line[0] == 'X' and len(line[1:]) >= 2:
            if len(line[1:]) == 2:
                score = convert_symbol_to_score(line[0]) + calc_two_try_score(line[1:3])
            else:
                score = convert_symbol_to_score(line[0]) + calc_two_try_score(line[1:3]) + splits_happen(line[1:])

        if line[0] in "0123456789-":
            score = convert_symbol_to_score(line[0]) + splits_happen(line[1:])

        if line[0] in "0123456789" and line[1] == '/':
            t1 = convert_symbol_to_score(line[0])
            if len(line[2:]) == 1:
                score = calc_two_try_score(line[0:2]) + convert_symbol_to_score(line[2])
            else:
                score = calc_two_try_score(line[0:2]) + convert_symbol_to_score(line[2]) + splits_happen(line[2:])
